...
class GeminiReasoningService(private val logger: Logger) : ReasoningService {
    ...
    override suspend fun generateDailyNarrative(
        ...
        val fallbackUrl = "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=$apiKey"
        ...
        val systemPrompt = """
            ...
            {
              "summary": "A concise, active-voice sentence describing the main activity.",
              "cognitiveMode": "One of: Deep Focus, Communication, Information Seeking, Administrative, Creative, or Casual.",
              "actionItems": ["List of detected tasks or follow-ups, or empty list if none."]
            }
        ...
        val userPrompt = buildString {
            ...
            if (!previousDaySummary.isNullOrBlank()) {
        ...
        return try {
            ...
            if (response.status == HttpStatusCode.OK) {
                ...
                Result.success(AppDailyReasoning(
                    ...
                    actionItems = parsed["actionItems"]?.jsonArray?.map { it.jsonPrimitive.content } ?: emptyList()
                ))
            } else {
                val errorBody = response.bodyAsText()
                logger.w { "Gemini API returned ${response.status}: $errorBody. Falling back to local reasoning." }
                LocalHeuristicService().generateDailyNarrative(packageName, appName, date, ocrDumps, labels, previousDaySummary)
            }
        } catch (e: Exception) {
