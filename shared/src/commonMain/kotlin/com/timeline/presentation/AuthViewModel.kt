...
class AuthViewModel(
    ...
    private val authUiHelper: AuthUiHelper,
        ...
        when (event) {
            ...
            is AuthEvent.SignInWithGoogle -> {
                viewModelScope.launch {
                    ...
                    authUiHelper.getGoogleIdToken(event.context)
                        .onSuccess { token ->
                            processGoogleIdToken(token)
                        }
                        .onFailure { error ->
                            _state.update { it.copy(isLoading = false, error = error.message ?: "Cancelled") }
                        }
                }
            }
    ...
    private suspend fun processGoogleIdToken(idToken: String) {
